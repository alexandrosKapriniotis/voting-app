...
>
    ...
    <div
        class="absolute z-10 md:w-104 w-64 text-left font-semibold text-sm shadow-dialog rounded-xl mt-2 bg-white"
        x-cloak
    ...
    >
        @auth
            <form wire:submit.prevent="addComment" class="space-y-4 px-4 py-6 bg-white">
                ...
                <div class="flex flex-col md:flex-row items-center md:space-x-3">
                    <button type="submit" class="text-sm flex items-center justify-center h-11 w-full md:w-1/2 text-xs bg-blue
                                font-semibold rounded-xl border border-blue hover:bg-blue-hover transition duration-150 ease-in px-6 py-3 text-white">
                        Post Comment
                    </button>
        ...
        @else
            <div class="px-4 py-6 text">
                <p class="font-normal">Please login or create an account to post a comment</p>
                <div class="flex items-center space-x-3 mt-8">
                    <a href="{{ route('login') }}" class="flex items-center justify-center w-1/2 h-11 text-xs bg-blue
                        font-semibold rounded-xl border border-blue hover:bg-blue-hover transition duration-150 ease-in px-6 py-3 text-white">
                        <span class="ml-2">
                            Login
                        </span>
                    </a>
                    <a href="{{ route('register') }}" class="flex items-center justify-center w-1/2 h-11 text-xs bg-gray-200
